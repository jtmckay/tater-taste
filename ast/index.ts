...
program
  .description("An application for exploring typescript repositories")
  .version("1.0.0")
  .addHelpText(
    "after",
    `
    ...
    (filePath: string, { config, map }: { config: string; map: string }) => {
      try {
        ...
        fs.writeFileSync(
          ...
          JSON.stringify(ast.fileGraph, null, 2)
        );
        fs.writeFileSync(
          join(__dirname, "../client/src/", "sourceFiles.json"),
          JSON.stringify(ast.sourceFiles, null, 2)
        );
        console.log(
          "Output",
          join(__dirname, "../client/src/", "sourceFiles.json")
